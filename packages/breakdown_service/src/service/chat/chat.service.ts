...
import { Chat } from '@breakdownrescue/database';

export const getChatsForRequest = async (requestId: number): Promise<Chat[]> => {
  try {
...
export const getChatsForDriverAndRequest = async (driverId: number, requestId: number): Promise<Chat[]> => {
  try {
    return await ChatRepository.getChatsForDriverAndRequest(driverId, requestId);
  } catch (error) {
