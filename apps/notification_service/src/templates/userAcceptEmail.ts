import { notificationIcons } from "@towmycar/common";
...
export const userAcceptEmail = (payload: any) => {
  const content = `
    ...
    <h2>More details:</h2>
    <ul>
    <li><strong>Reference Id:</strong> ${payload.requestId}</li>
    </ul> 
  ...
  return {
    subject: `${notificationIcons.USER_ACCEPTED} Action Required: Confirm Service for Request - Reference Id: ${payload.requestId}`,
    htmlBody,
  };
};
