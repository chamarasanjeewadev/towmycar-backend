...
export const generateKey = (userId:number,type:UploadDocumentType) => {
  switch(type){
    case UploadDocumentType.DRIVER_LICENSE_FRONT:
      return `${userId}/driver_license_front.pdf`;
    case UploadDocumentType.DRIVER_LICENSE_BACK:
      return `${userId}/driver_license_back.pdf`;
    ...
    case UploadDocumentType.VEHICLE_PHOTO:
      return `${userId}/vehicle_photo.pdf`;
...
export async function generateFilePath(userId:number,documentType:UploadDocumentType) {
